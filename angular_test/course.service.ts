...
export class CourseService {
  ...
  deleteOnCourse(course: Course): Observable<any> {
    const url = `${this.coursesUrl}/${course.id}`;
    return this.http.put<any>(url, course)
      .pipe(
        tap( _ => this.log(`El curso ${course.name} dejo de estar en desarrollo`)),
        catchError(this.handleError<Course>("deleteOnCourse"))
      );
  }

  addOnCourse(course: Course): Observable<any> {
    const url = `${this.coursesUrl}/${course.id}`;
    return this.http.put<any>(url, course)
      .pipe(
        tap( _ => this.log(`El curso ${course.name} está desarrollo`)),
        catchError(this.handleError<Course>("addOnCourse"))
      );
  }
